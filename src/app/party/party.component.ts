...
export class PartyComponent implements OnInit {
	...
	displayWinFrame(): boolean {
		if (
			this.manchePlayer >= this.gameService.getNbRound() ||
			this.mancheBot >= this.gameService.getNbRound()
		) {
	...
	getPlayerInfo(): any {
		if (!this.gameService.getCode2()) {
			...
			return (
				Math.round(
					(this.scorePlayer / (this.scorePlayer + this.scoreBot)) *
						100
				) + '%'
			);
		}
	...
	getBotInfo(): any {
		if (!this.gameService.getCode2()) {
			...
			return (
				Math.round(
					(this.scoreBot / (this.scorePlayer + this.scoreBot)) * 100
				) + '%'
			);
		}
	}
	...
	countdownTimer(): void {
		let count = 5;
		const preLoadSound = new Audio(
			'.\\assets\\sounds\\SFB-recharge_bullet_02.mp3'
		);
	...
	getWinner(): void {
		...
		const buttonSound = new Audio(
			'.\\assets\\sounds\\ButtonClickPoker.mp3'
		);
		buttonSound.play();
